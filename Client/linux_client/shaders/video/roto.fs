/* WASN"T ABLE TO GET THIS ONE WORKING :(
===========================================================================
...
void main(void) {
	float u_BlurMagnitude = 5.0;
	vec2 r_FBufScale = vec2(640.0,480.0);
	...
	*/
	vec2 r_NPOTScale = vec2((640.0/512.0),(480.0/512.0));
...
//	vec2 st00 = gl_FragCoord.st;
	...
	// base color
	vec4 c00 = texture2D(img, st00);

	// sample the current render for each coordinate
	vec4 c01 = texture2D(img, st01);
	vec4 c02 = texture2D(img, st02);
	vec4 c03 = texture2D(img, st03);
	vec4 c04 = texture2D(img, st04);
	vec4 c05 = texture2D(img, st05);
	vec4 c06 = texture2D(img, st06);
	vec4 c07 = texture2D(img, st07);
	vec4 c08 = texture2D(img, st08);
